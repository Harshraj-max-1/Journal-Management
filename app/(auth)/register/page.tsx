...
export default function RegisterPage() {
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 bg-[var(--background)] transition-colors duration-300 relative overflow-hidden min-h-screen">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[60vw] h-[60vw] bg-indigo-100 dark:bg-indigo-900/20 rounded-full blur-[120px] -z-10 transition-colors duration-300"></div>
      <div className="absolute bottom-[-20%] right-[-10%] w-[50vw] h-[50vw] bg-violet-100 dark:bg-violet-900/20 rounded-full blur-[100px] -z-10 transition-colors duration-300"></div>

      <div ref={cardRef} className="w-full max-w-xl bg-[var(--surface)] p-12 rounded-[56px] shadow-2xl shadow-indigo-100/50 dark:shadow-none border border-[var(--card-border)] relative z-10 transition-all duration-300">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-8">
          ...
          <div className="col-span-2 space-y-3">
            ...
            <div className="relative">
              <select 
                ...
                className="w-full bg-[var(--input-bg)] border-2 border-[var(--input-border)] text-[var(--on-background)] p-6 rounded-2xl font-black uppercase text-[10px] tracking-widest focus:outline-none focus:border-[var(--primary)] appearance-none cursor-pointer transition-all shadow-sm"
              >
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="col-span-2 py-6 bg-[var(--primary)] text-white font-black uppercase tracking-widest text-xs rounded-3xl shadow-2xl hover:-translate-y-1 active:scale-95 transition-all text-center flex items-center justify-center gap-3 disabled:opacity-70 disabled:hover:translate-y-0"
          >
